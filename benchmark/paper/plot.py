...
# cycled.  Validated against a white page surface with the six checks
# (OKLab dE x100, Machado 2009 CVD sim):
#
#   3-series adjacent + all-pairs : CVD 9.2 (deutan), normal 24.0   PASS
#   4-series adjacent             : CVD 9.1 (protan), normal 22.9   PASS
#   diverging blue<->red          : CVD 21.6,         normal 32.3   PASS
#
...
def main(argv=None):
    ...
    p.add_argument("--only", action="append", choices=list(FIGURES),
    ...
    for name in wanted:
        ...
        made = FIGURES[name](args.out, args)
        if made is None:
