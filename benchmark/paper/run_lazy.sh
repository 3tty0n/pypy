...
TSV="$OUT/lazy.tsv"
tsv_init "$TSV" "model\tsystem\tround\tsteady_us\tlaunches_per_iter\tkernels\tcompiles\tgc_bytes_per_iter\tlazy_nodes\tlazy_forces\tlazy_barriers\tlazy_pruned\tlazy_fallbacks\tfirst_run_ms\tchecksum\targmax_match\tbinary"
...
row() {
  ...
  local steady launches kern comp gcb nodes forces barriers pruned fallbacks first cks
  ...
  pruned=$(field_of "$out" lazy_pruned)
  fallbacks=$(field_of "$out" lazy_fallbacks)
  first=$(field_of "$out" first_run_ms)
  cks=$(field_of "$out" checksum)
  echo -e "$model\t$system\t$round\t$steady\t$launches\t$kern\t$comp\t$gcb\t$nodes\t$forces\t$barriers\t$pruned\t$fallbacks\t$first\t$cks\t$match\t${PYPY_SHA:-unknown}" >> "$TSV"
