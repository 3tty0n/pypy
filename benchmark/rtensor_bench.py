...
def entry_point(argv):
    ...
    kernels.init_device()
    try:
        cfg.dtype = core.dtype_of_name(_bench_env('RTENSOR_DTYPE',
                                                     'float64'))
    except ValueError:
    ...
    if variant >= 6:
        ...
        print '%s %d %d %d %d %f %f %d %f %d %f %s' % (mode, variant, k, n,
            iters, warm, steady, kernels.counter.n, acc,
    ...
    print '%s %d %d %d %d %f %f %d %f %d %f %s' % (mode, variant, k, n, iters,
        warm, steady, kernels.counter.n, acc, kernels.counter.n - before,
        launches, dtname)
    return 0
