# -*- coding: utf-8 -*-
# Generated by benchmark/suites/port.py from timm.models.beit
# (timm 1.0.22, source sha256 e2b3652fcccdf39a).  Do not edit; change the spec.
# Kept: gen_relative_position_index, Attention, Block, RelativePositionBias, Beit, checkpoint_filter_fn, _create_beit, beit_base_patch16_224
from __future__ import absolute_import, division, print_function
import math
import torch
import torch.nn as nn
import torch.nn.functional as F
from timm.layers import PatchEmbed, Mlp, SwiGLU, LayerNorm, DropPath, calculate_drop_path_rates, trunc_normal_, use_fused_attn, resample_patch_embed, resample_abs_pos_embed, resize_rel_pos_bias_table, ndgrid
from timm.models._builder import build_model_with_cfg
from timm.models._features import feature_take_indices
from timm.models._manipulate import checkpoint
from timm.models._registry import register_model

def gen_relative_position_index(window_size, device=None):
    ...
    coords = torch.stack(ndgrid(torch.arange(window_size[0], device=device, dtype=torch.long), torch.arange(window_size[1], device=device, dtype=torch.long)))
...
class Attention(nn.Module):
    ...
    def __init__(self, dim, num_heads=8, qkv_bias=False, qkv_bias_separate=False, attn_drop=0.0, proj_drop=0.0, window_size=None, attn_head_dim=None, device=None, dtype=None):
        ...
        if window_size:
            ...
            self.relative_position_bias_table = nn.Parameter(torch.zeros(self.num_relative_distance, num_heads, **dd))
            self.register_buffer('relative_position_index', gen_relative_position_index(window_size, device=device), persistent=False)
        else:
    ...
    def forward(self, x, shared_rel_pos_bias=None):
        ...
        if self.fused_attn:
            ...
            x = F.scaled_dot_product_attention(q, k, v, attn_mask=rel_pos_bias, dropout_p=self.attn_drop.p if self.training else 0.0)
        else:
            q = q * self.scale
...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, qkv_bias=False, mlp_ratio=4.0, scale_mlp=False, swiglu_mlp=False, proj_drop=0.0, attn_drop=0.0, drop_path=0.0, init_values=None, act_layer=nn.GELU, norm_layer=LayerNorm, window_size=None, attn_head_dim=None, device=None, dtype=None):
        ...
        self.attn = Attention(dim, num_heads=num_heads, qkv_bias=qkv_bias, attn_drop=attn_drop, proj_drop=proj_drop, window_size=window_size, attn_head_dim=attn_head_dim, **dd)
        self.drop_path1 = DropPath(drop_path) if drop_path > 0.0 else nn.Identity()
        self.norm2 = norm_layer(dim, **dd)
        if swiglu_mlp:
            self.mlp = SwiGLU(in_features=dim, hidden_features=int(dim * mlp_ratio), norm_layer=norm_layer if scale_mlp else None, drop=proj_drop, **dd)
        else:
            self.mlp = Mlp(in_features=dim, hidden_features=int(dim * mlp_ratio), act_layer=act_layer, norm_layer=norm_layer if scale_mlp else None, drop=proj_drop, **dd)
...
class RelativePositionBias(nn.Module):
    ...
    def __init__(self, window_size, num_heads, device=None, dtype=None):
...
class Beit(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, global_pool='avg', embed_dim=768, depth=12, num_heads=12, qkv_bias=True, mlp_ratio=4.0, swiglu_mlp=False, scale_mlp=False, drop_rate=0.0, pos_drop_rate=0.0, proj_drop_rate=0.0, attn_drop_rate=0.0, drop_path_rate=0.0, norm_layer=LayerNorm, init_values=None, use_abs_pos_emb=True, use_rel_pos_bias=False, use_shared_rel_pos_bias=False, head_init_scale=0.001, device=None, dtype=None):
        ...
        self.patch_embed = PatchEmbed(img_size=img_size, patch_size=patch_size, in_chans=in_chans, embed_dim=embed_dim, **dd)
...
def _create_beit(variant, pretrained=False, **kwargs):
    ...
    model = build_model_with_cfg(Beit, variant, pretrained, pretrained_filter_fn=checkpoint_filter_fn, feature_cfg=dict(out_indices=out_indices, feature_cls='getter'), **kwargs)
    return model

@register_model
def beit_base_patch16_224(pretrained=False, **kwargs):
    """BEiT base model @ 224x224 with patch size 16x16."""
    model_args = dict(patch_size=16, embed_dim=768, depth=12, num_heads=12, mlp_ratio=4, use_abs_pos_emb=False, use_rel_pos_bias=True, init_values=0.1)
