# -*- coding: utf-8 -*-
# Generated by benchmark/suites/port.py from torchbenchmark.models.pyhpc_turbulent_kinetic_energy.tke_pytorch
# (torchbenchmark git 0fff8bf354, source sha256 135f38929c8f5bb4).  Do not edit; change the spec.
# Kept: integrate_tke
...
def solve_implicit(ks, a, b, c, d, b_edge):
    land_mask = (ks >= 0)[:, :, None]
    edge_mask = land_mask & (torch.arange(a.shape[2], device=ks.device)[None, None, :] == ks[:, :, None])
    water_mask = land_mask & (torch.arange(a.shape[2], device=ks.device)[None, None, :] >= ks[:, :, None])
...
def limiter(cr):
    return torch.maximum(torch.tensor([0.0], device=cr.device), torch.maximum(torch.minimum(torch.tensor([1.0], device=cr.device), 2 * cr), torch.minimum(torch.tensor([2.0], device=cr.device), cr)))
...
def _adv_superbee(vel, var, mask, dx, axis, cost, cosu, dt_tracer):
    if axis == 0:
        ...
        return vel[1:-2, 2:-2, :] * (var[2:-1, 2:-2, :] + var[1:-2, 2:-2, :]) * 0.5 - torch.abs(vel[1:-2, 2:-2, :]) * (1.0 - cr + uCFL * cr) * rj * 0.5
    elif axis == 1:
        ...
        return velfac * vel[2:-2, 1:-2, :] * (var[2:-2, 2:-1, :] + var[2:-2, 1:-2, :]) * 0.5 - torch.abs(velfac * vel[2:-2, 1:-2, :]) * (1.0 - cr + uCFL * cr) * rj * 0.5
    elif axis == 2:
        ...
        return vel[2:-2, 2:-2, 1:-2] * (var[2:-2, 2:-2, 2:-1] + var[2:-2, 2:-2, 1:-2]) * 0.5 - torch.abs(vel[2:-2, 2:-2, 1:-2]) * (1.0 - cr + uCFL * cr) * rj * 0.5
    else:
...
def integrate_tke(u, v, w, maskU, maskV, maskW, dxt, dxu, dyt, dyu, dzt, dzw, cost, cosu, kbot, kappaM, mxl, forc, forc_tke_surface, tke, dtke):
    ...
    sqrttke = torch.sqrt(torch.maximum(torch.tensor([0.0], device=tke.device), tke[:, :, :, tau]))
    ...
    delta[:, :, :-1] = dt_tke / dzt[None, None, 1:] * alpha_tke * 0.5 * (kappaM[2:-2, 2:-2, :-1] + kappaM[2:-2, 2:-2, 1:])
    a_tri[:, :, 1:-1] = -delta[:, :, :-2] / dzw[None, None, 1:-1]
    a_tri[:, :, -1] = -delta[:, :, -2] / (0.5 * dzw[-1])
    b_tri[:, :, 1:-1] = 1 + (delta[:, :, 1:-1] + delta[:, :, :-2]) / dzw[None, None, 1:-1] + dt_tke * c_eps * sqrttke[2:-2, 2:-2, 1:-1] / mxl[2:-2, 2:-2, 1:-1]
    b_tri[:, :, -1] = 1 + delta[:, :, -2] / (0.5 * dzw[-1]) + dt_tke * c_eps / mxl[2:-2, 2:-2, -1] * sqrttke[2:-2, 2:-2, -1]
    b_tri_edge = 1 + delta / dzw[None, None, :] + dt_tke * c_eps / mxl[2:-2, 2:-2, :] * sqrttke[2:-2, 2:-2, :]
    ...
    tke_surf_corr[2:-2, 2:-2] = torch.where(mask, -tke[2:-2, 2:-2, -1, taup1] * 0.5 * dzw[-1] / dt_tke, 0.0)
    tke[2:-2, 2:-2, -1, taup1] = torch.maximum(torch.tensor([0.0], device=tke.device), tke[2:-2, 2:-2, -1, taup1])
    '\n    add tendency due to lateral diffusion\n    '
    flux_east[:-1, :, :] = K_h_tke * (tke[1:, :, :, tau] - tke[:-1, :, :, tau]) / (cost[None, :, None] * dxu[:-1, None, None]) * maskU[:-1, :, :]
    flux_east[-1, :, :] = 0.0
    flux_north[:, :-1, :] = K_h_tke * (tke[:, 1:, :, tau] - tke[:, :-1, :, tau]) / dyu[None, :-1, None] * maskV[:, :-1, :] * cosu[None, :-1, None]
    flux_north[:, -1, :] = 0.0
    tke[2:-2, 2:-2, :, taup1] += dt_tke * maskW[2:-2, 2:-2, :] * ((flux_east[2:-2, 2:-2, :] - flux_east[1:-3, 2:-2, :]) / (cost[None, 2:-2, None] * dxt[2:-2, None, None]) + (flux_north[2:-2, 2:-2, :] - flux_north[2:-2, 1:-3, :]) / (cost[None, 2:-2, None] * dyt[None, 2:-2, None]))
    '\n    add tendency due to advection\n    '
    adv_flux_superbee_wgrid(flux_east, flux_north, flux_top, tke[:, :, :, tau], u[..., tau], v[..., tau], w[..., tau], maskW, dxt, dyt, dzw, cost, cosu, dt_tracer)
    dtke[2:-2, 2:-2, :, tau] = maskW[2:-2, 2:-2, :] * (-(flux_east[2:-2, 2:-2, :] - flux_east[1:-3, 2:-2, :]) / (cost[None, 2:-2, None] * dxt[2:-2, None, None]) - (flux_north[2:-2, 2:-2, :] - flux_north[2:-2, 1:-3, :]) / (cost[None, 2:-2, None] * dyt[None, 2:-2, None]))
