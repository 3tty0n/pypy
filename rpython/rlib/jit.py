...
PARAMETER_DOCS = {
    ...
    'enable_hbp_class_promotion': 'also let HBP promote guard_class/guard_nonnull_class failures keyed by observed class (0=off, 1=on)',
    'enable_hbp_bool_promotion': 'also let HBP promote optimizer-created guard_true/guard_false failures that came from guard_value(bool, const) (0=off, 1=on)',
    'enable_hbp_guard_bool_promotion': 'also let HBP promote ordinary guard_true/guard_false failures keyed by the failing boolean value (0=off, 1=on)',
    'enable_hbp_nullness_promotion': 'also let HBP promote guard_nonnull/guard_isnull failures keyed by the failing nullness branch (0=off, 1=on)',
    'enable_hbp_value_promotion': 'let HBP promote ordinary integer guard_value failures keyed by the failing value (0=off, 1=on; default on)',
    'enable_hbp_ref_value_promotion': 'also let HBP promote ordinary reference guard_value failures keyed by object identity (0=off, 1=on; default on)',
    'enable_hbp_float_value_promotion': 'also let HBP promote ordinary float guard_value failures keyed by value hash (0=off, 1=on; default on)',
    'hot_bridge_threshold': 'minimum ordinary bridge count for the loop (excluding HBP variants) before HBP considers promoting a failing guard (default 1)',
    'hot_bridge_guard_threshold': 'minimum per-guard failure count (rd_fail_count) before HBP promotes THAT guard to a loop variant (default 50; sustained-vs-transient discriminator: a genuinely hot guard keeps failing while a near-monomorphic loop transient guard plateaus low)',
    'hot_bridge_value_threshold': 'minimum exact-value failure count before HBP promotes guard_value-style integer/ref/float failures (0=use hot_bridge_guard_threshold; default 0)',
    'hot_bridge_int_value_threshold': 'minimum exact-value failure count before HBP promotes integer guard_value failures (0=use hot_bridge_value_threshold/hot_bridge_guard_threshold; default 0)',
    'hot_bridge_min_value_share_pct': 'minimum percentage of this guard failures that must have the same exact value before HBP specializes that value (0=off; default 0)',
    'hot_bridge_guard_bool_threshold': 'minimum per-guard failure count before HBP promotes an ordinary guard_true/guard_false failure (default 200; optimizer-created bool value guards still use hot_bridge_guard_threshold)',
    'hot_bridge_nullness_threshold': 'minimum per-guard failure count before HBP promotes guard_nonnull/guard_isnull failures (default 200)',
    'hot_bridge_max_variants': 'maximum loop variants HBP will peel for one loop before declaring it megamorphic and falling back to generic bridges (default 8; bounds blow-up even if the per-guard discriminator mis-classifies a megamorphic-dispatch guard as hot)',
    'hot_bridge_max_value_variants': 'maximum exact-value HBP variants per loop (0=unlimited; bool/class variants are not counted; default 0)',
    'hot_bridge_max_bool_variants': 'maximum bool-keyed HBP variants per loop (0=unlimited; default 0)',
    'hot_bridge_max_ref_value_variants': 'maximum reference exact-value HBP variants per loop (0=unlimited; default 0)',
    'hot_bridge_max_cardinality': 'maximum distinct guard_value values (popcount of rd_value_sig) for which HBP still peels a loop variant (default 4; megamorphic guards saturate past it and fall back to generic bridges)',
    'hot_bridge_min_candidates': 'minimum distinct HBP-eligible guard sites in one loop before promoting a bridge variant (default 1; higher values avoid specializing isolated one-off exits)',
    'hot_bridge_max_candidates': 'maximum distinct HBP-eligible guard sites in one loop (0=unlimited; default 0)',
    'hot_bridge_max_ref_candidates': 'maximum distinct ref-value HBP-eligible guard sites in one loop (0=use hot_bridge_max_candidates/unlimited; default 0)',
    'hot_bridge_max_loop_bridges': 'maximum ordinary bridge count for a loop before HBP stops promoting new variants there (0=unlimited; use to avoid specializing broad high-fanout loops)',
    'hot_bridge_global_max_variants': 'maximum HBP-specialized loop variants for one process/JIT driver before falling back to generic bridges (0=unlimited; default 0)',
    'hot_bridge_global_max_ref_traces': 'maximum ref-value HBP trace attempts for one process/JIT driver before falling back to generic bridges (0=unlimited; default 0)',
    'hot_bridge_ref_bridge_threshold': 'minimum ordinary bridges in a loop before ref-value HBP promotion is allowed (0=use hot_bridge_threshold; default 0)',
    'hot_bridge_class_bridge_threshold': 'minimum ordinary bridges in a loop before class-keyed HBP promotion is allowed (0=use hot_bridge_threshold; default 0)',
    'hot_bridge_max_ops': 'maximum optimized operations in an HBP-specialized bridge variant before falling back to the generic bridge path (0=unlimited; default 0)',
    'hot_bridge_max_guards': 'maximum guards in an HBP-specialized bridge variant before falling back to the generic bridge path (0=unlimited; default 0)',
    'hot_bridge_min_op_reduction': 'minimum raw-to-optimized operation reduction required for an HBP-specialized bridge variant (0=off; default 0)',
    'hot_bridge_min_guard_reduction': 'minimum raw-to-optimized guard reduction required for an HBP-specialized bridge variant (0=off; default 0)',
    'hot_bridge_min_op_reduction_pct': 'minimum percent raw-to-optimized operation reduction required for an HBP-specialized bridge variant (0=off; default 0)',
    'hot_bridge_min_guard_reduction_pct': 'minimum percent raw-to-optimized guard reduction required for an HBP-specialized bridge variant (0=off; default 0)',
    'hbp_entry_guard': 'emit an explicit entry guard_value for exact int/ref HBP variants, then optimize behind that guard (0=off, 1=on; default off)',
    'hbp_entry_guard_ref_bridge_threshold': 'minimum ordinary bridges in a loop before exact ref HBP variants get an explicit entry guard (0=off; default 0)',
    'hbp_bool_entry_guard': 'also emit explicit entry guards for bool HBP variants when the bool is already a bridge failarg (0=off, 1=on; experimental, default off)',
    'hbp_value_counter_slots': 'number of heavy-hitter slots used to count exact-value HBP failures per guard. 1 preserves consecutive-value counting; 2 tracks two recurring values through alternation (default 1)',
    'enable_hbp_cardinality_gate': 'apply the hot_bridge_max_cardinality popcount gate to HBP promotion (0=off, 1=on; default on; off makes the cardinality clause vacuously true)',
    'retrace_min_loop_bridges': 'minimum ordinary bridge count before ordinary retracing is allowed when retrace_limit > 0 (0=stock retrace_limit behavior; HBP-promoted variants are not blocked)',
    'retrace_min_ops': 'minimum optimized operation count before ordinary retracing is allowed when retrace_limit > 0 (0=stock retrace_limit behavior; HBP-promoted variants are not blocked)',
    ...
    'loop_unroll_metric': 'runtime A/B unroll: decision metric, 1=min (fastest interleaved entry, sensitive), 0=mean (representative), or 2=robust (adopt fK only if min AND mean both favour it; rejects lucky-min false adoptions on allocation-heavy loops). Only used when loop_unroll_factor>1.',
    ...
    'loop_unroll_numeric_only': 'runtime A/B unroll: when 1, only loops with no residual calls (tight numeric loops) enter the A/B trial; object-churn loops are excluded so they never pay the trial overhead. 0=trial every hot loop. Only used when loop_unroll_factor>1.',
    'loop_unroll_phase': 'runtime A/B unroll: 0=interleave f1/fK per entry (cancels drift but never runs a variant continuously); >0=run this many consecutive entries of one variant before switching, so the trial measures each variant true steady-state cost (catches fK regressions that only appear under continuous execution). Only used when loop_unroll_factor>1.',
    'loop_unroll_max_ops': 'runtime A/B unroll: 0=off; >0 => a loop only enters the trial if its estimated unrolled body (f1 optimized op count * factor) is at most this, so large loops are never unrolled (avoids the i-cache bloat that makes unrolling a net loss on big loops). Only used when loop_unroll_factor>1.',
}
