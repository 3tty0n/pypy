...
from rpython.jit.metainterp.history import (Const, ConstInt, ConstPtr,
    ConstFloat, CONST_NULL, CONST_FALSE, CONST_TRUE, getkind, AbstractDescr)
...
_SAME_BOX_RESULT = {
    'INT_EQ': 'CONST_TRUE', 'INT_NE': 'CONST_FALSE',
    'INT_LT': 'CONST_FALSE', 'INT_LE': 'CONST_TRUE',
    'INT_GT': 'CONST_FALSE', 'INT_GE': 'CONST_TRUE',
}


def _same_box_result(rop_name):
    return _SAME_BOX_RESULT[rop_name]
...
class GenExtension(object):
    ...
    def generate(self):
        ...
        d = {"ConstInt": ConstInt, "ConstPtr": ConstPtr, "ConstFloat": ConstFloat, "JitCode": JitCode, "ChangeFrame": ChangeFrame,
             ...
             'rop': rop, 'CONST_FALSE': CONST_FALSE,
    ...
    def emit_newframe_function(self):
        return ["self._result_argcode = %r" % (self.returncode, ),
                "self._try_tco()",
                "return # change frame"]
...
class Specializer(object):
    ...
    def _emit_unspecialized_int_comparison_fast(self, rop_name, py_op):
        ...
        lines.append("# fast-path recording, skip heapcache")
        lines.append("_op = self._shortcut_record_cmp(rop.%s, %s, %s, %s, _res)" % (
            rop_name, box0, box1, _same_box_result(rop_name)))
        lines.append("self.registers_i[%d] = _op" % result.index)
        lines.append("i%d = _res" % result.index)
        next_consts = self.constant_registers - {result}
        self._emit_jump(lines, constant_registers=next_consts)
    ...
    def _get_constant_int_value(self, arg):
        if isinstance(arg, Constant) and arg.concretetype in (lltype.Signed,
                                                              lltype.Bool):
    ...
    def _emit_goto_if_not_int_comparison_fast(self, rop_name, py_op):
        ...
        lines.append("condbox = self._shortcut_record_cmp(rop.%s, %s, %s, %s, _cond)" % (
            rop_name, box0, box1, _same_box_result(rop_name)))
...
def _make_register_syncer(constant_registers, cache={}):
    ...
    # L1-generated SSAReprs use fresh Register objects, so the same
    # (kind, index) can appear twice in the frozenset.
    by_slot = dict(((reg.kind, reg.index), reg)
                   for reg in constant_registers)
    constant_registers = sorted(by_slot.values(),
                                key=lambda reg: (reg.kind, reg.index))
