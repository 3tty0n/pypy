...
def test_genext_compile_function_target_gated(enable_genextension):
    # compile_shortcut emits x86-64 machine code; on a non-x86-64 target
    # genext_compile_function must stay None (pure-arith loops then use the
    # normal backend path). genext_is_pure_arithmetic / genext_function are
    # unaffected. Host-aware so it is correct on x86-64 and AArch64 alike.
