...
def compile_simple_and_split(metainterp, greenkey, start,
                             inputargs, jumpargs, resumekey):
    """Try to compile a trace which is generated by a baseline (or, method)
    JIT tracer
    """
    metainterp_sd = metainterp.staticdata
    jitdriver_sd = metainterp.jitdriver_sd
    ...
    try:
        ...
        (body_info, body_ops), (bridge_info, bridge_ops) = \
            data.split(metainterp_sd, jitdriver_sd, metainterp.box_names_memo,
                       ops, loop_info.inputargs, fname="emit_jump", gmark="is_true",
                       body_token=body_start_descr, bridge_token=bridge_start_descr)
    except InvalidLoop:
        metainterp_sd.jitlog.trace_aborted()
        trace.cut_at(cut_at)
        return None

    logger.log_loop(body_info.inputargs, body_ops)
    logger.log_bridge(bridge_info.inputargs, bridge_ops)
...
def compile_loop(metainterp, greenkey, start, inputargs, jumpargs,
    ...
    jitcell_token = make_jitcell_token(jitdriver_sd)
    cut_at = history.get_trace_position()
    history.record(rop.JUMP, jumpargs, None, descr=jitcell_token) # XXX
    if start != (0, 0, 0):
