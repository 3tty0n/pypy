...
class Profiler(BaseProfiler):
    ...
    def start(self):
        ...
        self.current = []
        # Experiment: PYPY_JIT_FIXED_COST=1 replaces the timed fits with
        # the suite medians (0.7 us/failure, 132 us + 0.76 us/op per
        # bridge) so nondeterminism can be attributed to the clocks.
        self.fixed_cost = os.environ.get("PYPY_JIT_FIXED_COST") is not None
    ...
    def start_portal_call(self, insns):
        ...
        # Nested calls are stacked so each span is charged once.
        self.counters[Counters.BLACKHOLE_CALL] += 1
        if self.bh_call_t0 is None:
            self.bh_call_t0 = []
            self.bh_call_nested = []
        self.bh_call_t0.append(self.walltimer())
        self.bh_call_nested.append(0.0)

    def end_portal_call(self, insns):
        elapsed = self.walltimer() - self.bh_call_t0.pop()
        nested = self.bh_call_nested.pop()
        self.times[Counters.BLACKHOLE_CALL] += elapsed - nested
        if self.bh_call_nested:
            self.bh_call_nested[-1] += elapsed
    ...
    def bridge_cost(self, tail_ops):
        ...
        if tail_rec < 0.0:
            return -1.0
        if self.fixed_cost:
            return 132e-6 + 0.76e-6 * tail_rec
