# -*- coding: utf-8 -*-
# Generated by benchmark/suites/port.py from timm.layers.classifier
# (timm 1.0.22, source sha256 5dff2312d7c0cf0d).  Do not edit; change the spec.
# Kept: create_classifier, ClassifierHead, NormMlpClassifierHead, ClNormMlpClassifierHead
...
def _create_pool(num_features, num_classes, pool_type='avg', use_conv=False, input_fmt=None):
...
def create_classifier(num_features, num_classes, pool_type='avg', use_conv=False, input_fmt='NCHW', drop_rate=None, device=None, dtype=None):
    global_pool, num_pooled_features = _create_pool(num_features, num_classes, pool_type, use_conv=use_conv, input_fmt=input_fmt)
    fc = _create_fc(num_pooled_features, num_classes, use_conv=use_conv, device=device, dtype=dtype)
...
class ClassifierHead(nn.Module):
    ...
    def __init__(self, in_features, num_classes, pool_type='avg', drop_rate=0.0, use_conv=False, input_fmt='NCHW', device=None, dtype=None):
        """
        Args:
        ...
        global_pool, fc = create_classifier(in_features, num_classes, pool_type, use_conv=use_conv, input_fmt=input_fmt, device=device, dtype=dtype)
    ...
    def reset(self, num_classes, pool_type=None):
        if pool_type is not None and pool_type != self.global_pool.pool_type:
            self.global_pool, self.fc = create_classifier(self.in_features, num_classes, pool_type=pool_type, use_conv=self.use_conv, input_fmt=self.input_fmt)
...
class NormMlpClassifierHead(nn.Module):
    ...
    def __init__(self, in_features, num_classes, hidden_size=None, pool_type='avg', drop_rate=0.0, norm_layer='layernorm2d', act_layer='tanh', device=None, dtype=None):
        """
        Args:
            in_features: The number of input features.
            num_classes:  The number of classes for the final classifier layer (output).
        ...
        if hidden_size:
            self.pre_logits = nn.Sequential(OrderedDict([('fc', linear_layer(in_features, hidden_size, **dd)), ('act', act_layer())]))
...
class ClNormMlpClassifierHead(nn.Module):
    ...
    def __init__(self, in_features, num_classes, hidden_size=None, pool_type='avg', drop_rate=0.0, norm_layer='layernorm', act_layer='gelu', input_fmt='NHWC', device=None, dtype=None):
        """
        Args:
            in_features: The number of input features.
            num_classes:  The number of classes for the final classifier layer (output).
        ...
        """
        dd = {'device': device, 'dtype': dtype}
        super(ClNormMlpClassifierHead, self).__init__()
        self.in_features = in_features
        self.hidden_size = hidden_size
        self.num_features = in_features
        ...
        self.norm = norm_layer(in_features, **dd)
        if hidden_size:
            self.pre_logits = nn.Sequential(OrderedDict([('fc', nn.Linear(in_features, hidden_size, **dd)), ('act', act_layer())]))
            self.num_features = hidden_size
        else:
            self.pre_logits = nn.Identity()
        self.drop = nn.Dropout(drop_rate)
    ...
    def reset(self, num_classes, pool_type=None, reset_other=False):
        if pool_type is not None:
            self.pool_type = pool_type
        if reset_other:
            self.pre_logits = nn.Identity()
