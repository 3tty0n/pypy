# -*- coding: utf-8 -*-
# Generated by benchmark/suites/port.py from timm.models._efficientnet_builder
# (timm 1.0.22, source sha256 084cf15cc4725fd9).  Do not edit; change the spec.
# Kept: BN_MOMENTUM_TF_DEFAULT, BN_EPS_TF_DEFAULT, get_bn_args_tf, resolve_bn_args, resolve_act_layer, round_channels, decode_arch_def, EfficientNetBuilder, efficientnet_init_weights
...
from timm.models._efficientnet_blocks import ConvBnAct, DepthwiseSeparableConv, InvertedResidual, CondConvResidual, EdgeResidual, UniversalInvertedResidual, MobileAttention
...
def _decode_block_str(block_str):
    ...
    block_args = dict(block_type=block_type, out_chs=int(options['c']), stride=int(options['s']), act_layer=act_layer)
    if block_type == 'ir':
        block_args.update(dict(dw_kernel_size=_parse_ksize(options['k']), exp_kernel_size=start_kernel_size, pw_kernel_size=end_kernel_size, exp_ratio=float(options['e']), se_ratio=float(options.get('se', 0.0)), noskip=skip is False, s2d=int(options.get('d', 0)) > 0))
        if 'cc' in options:
            block_args['num_experts'] = int(options['cc'])
    elif block_type == 'ds' or block_type == 'dsa':
        block_args.update(dict(dw_kernel_size=_parse_ksize(options['k']), pw_kernel_size=end_kernel_size, se_ratio=float(options.get('se', 0.0)), pw_act=block_type == 'dsa', noskip=block_type == 'dsa' or skip is False, s2d=int(options.get('d', 0)) > 0))
    elif block_type == 'er':
        block_args.update(dict(exp_kernel_size=_parse_ksize(options['k']), pw_kernel_size=end_kernel_size, exp_ratio=float(options['e']), force_in_chs=force_in_chs, se_ratio=float(options.get('se', 0.0)), noskip=skip is False))
    elif block_type == 'cn':
        block_args.update(dict(kernel_size=int(options['k']), skip=skip is True))
    elif block_type == 'uir':
        ...
        block_args.update(dict(dw_kernel_size_start=start_kernel_size, dw_kernel_size_mid=_parse_ksize(options['k']), dw_kernel_size_end=end_kernel_size, exp_ratio=float(options['e']), se_ratio=float(options.get('se', 0.0)), noskip=skip is False))
    elif block_type == 'mha':
        kv_dim = int(options['d'])
        block_args.update(dict(dw_kernel_size=_parse_ksize(options['k']), num_heads=int(options['h']), key_dim=kv_dim, value_dim=kv_dim, kv_stride=int(options.get('v', 1)), noskip=skip is False))
    elif block_type == 'mqa':
        kv_dim = int(options['d'])
        block_args.update(dict(dw_kernel_size=_parse_ksize(options['k']), num_heads=int(options['h']), key_dim=kv_dim, value_dim=kv_dim, kv_stride=int(options.get('v', 1)), noskip=skip is False))
    else:
...
def decode_arch_def(arch_def, depth_multiplier=1.0, depth_trunc='ceil', experts_multiplier=1, fix_first_last=False, group_size=None):
...
class EfficientNetBuilder:
    ...
    def __init__(self, output_stride=32, pad_type='', round_chs_fn=round_channels, se_from_exp=False, act_layer=None, norm_layer=None, aa_layer=None, se_layer=None, drop_path_rate=0.0, layer_scale_init_value=None, feature_location='', device=None, dtype=None):
        ...
        self.dd = {'device': device, 'dtype': dtype}
    ...
    def _make_block(self, ba, block_idx, block_count):
        ...
        if bt == 'ir':
            ...
            block = UniversalInvertedResidual(**dict(list(ba.items()) + [('layer_scale_init_value', self.layer_scale_init_value)]))
        elif bt == 'mqa':
            _log_info_if('  MobileMultiQueryAttention {}, Args: {}'.format(block_idx, str(ba)), self.verbose)
            block = MobileAttention(**dict(list(ba.items()) + [('use_multi_query', True)] + [('layer_scale_init_value', self.layer_scale_init_value)]))
        elif bt == 'mha':
            _log_info_if('  MobileMultiHeadAttention {}, Args: {}'.format(block_idx, str(ba)), self.verbose)
            block = MobileAttention(**dict(list(ba.items()) + [('layer_scale_init_value', self.layer_scale_init_value)]))
        else:
        ...
        for stack_idx, stack_args in enumerate(model_block_args):
            ...
            for block_idx, block_args in enumerate(stack_args):
                ...
                if extract_features:
                    ...
                    if leaf_name:
                        feature_info['module'] = '.'.join(['blocks.{0}.{1}'.format(stack_idx, block_idx), leaf_name])
                    else:
                        assert last_block
                        feature_info['module'] = 'blocks.{0}'.format(stack_idx)
                    self.features.append(feature_info)
                total_block_idx += 1
