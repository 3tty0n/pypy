# -*- coding: utf-8 -*-
# Generated by benchmark/suites/port.py from transformers.modeling_rope_utils
# (transformers 5.13.0, source sha256 9592ba129bf723ed).  Do not edit; change the spec.
# Kept: ROPE_INIT_FUNCTIONS, dynamic_rope_update
...
def dynamic_rope_update(rope_forward):
    ...
    def longrope_frequency_update(self, position_ids, device, layer_type=None):
        ...
        if layer_type is None:
            ...
            original_inv_freq = getattr(self, '{0}_original_inv_freq'.format(layer_type))
            prefix = '{0}_'.format(layer_type)
            original_max_position_embeddings = self.config.rope_parameters[layer_type]['original_max_position_embeddings']
        if seq_len > original_max_position_embeddings:
            if not hasattr(self, '{0}_long_inv_freq'.format(layer_type)):
                rope_init_fn = ROPE_INIT_FUNCTIONS[rope_type]
                long_inv_freq, _ = rope_init_fn(self.config, device, seq_len=original_max_position_embeddings + 1, layer_type=layer_type)
            self.register_buffer('{0}inv_freq'.format(prefix), long_inv_freq, persistent=False)
            setattr(self, '{0}long_inv_freq'.format(prefix), long_inv_freq)
        else:
            original_inv_freq = original_inv_freq.to(device)
            self.register_buffer('{0}inv_freq'.format(prefix), original_inv_freq, persistent=False)
            setattr(self, '{0}original_inv_freq'.format(prefix), original_inv_freq)
    ...
    def dynamic_frequency_update(self, position_ids, device, layer_type=None):
        ...
        if layer_type is None:
            ...
            rope_type = self.rope_type[layer_type]
            max_seq_len_cached = getattr(self, '{0}_max_seq_len_cached'.format(layer_type), self.max_seq_len_cached)
            original_inv_freq = getattr(self, '{0}_original_inv_freq'.format(layer_type))
            prefix = '{0}_'.format(layer_type)
        if seq_len > max_seq_len_cached:
            rope_init_fn = ROPE_INIT_FUNCTIONS[rope_type]
            inv_freq, self.attention_scaling = rope_init_fn(self.config, device, seq_len=seq_len, layer_type=layer_type)
            self.register_buffer('{0}inv_freq'.format(prefix), inv_freq, persistent=False)
            setattr(self, '{0}max_seq_len_cached'.format(prefix), seq_len)
        if seq_len < self.original_max_seq_len and max_seq_len_cached > self.original_max_seq_len:
            original_inv_freq = original_inv_freq.to(device)
            self.register_buffer('{0}inv_freq'.format(prefix), original_inv_freq, persistent=False)
            setattr(self, '{0}original_inv_freq'.format(prefix), original_inv_freq)
            setattr(self, '{0}max_seq_len_cached'.format(prefix), self.original_max_seq_len)
...
def _compute_linear_scaling_rope_parameters(config=None, device=None, seq_len=None, layer_type=None):
    """
...
def _compute_proportional_rope_parameters(config=None, device=None, seq_len=None, layer_type=None, head_dim_key='head_dim'):
    """
    ...
    inv_freq_rotated = 1.0 / base ** (torch.arange(0, 2 * rope_angles, 2, dtype=torch.int64).to(device=device, dtype=torch.float) / head_dim)
    nope_angles = head_dim // 2 - rope_angles
    if nope_angles > 0:
        inv_freq = torch.cat((inv_freq_rotated, torch.zeros(nope_angles, dtype=torch.float32, device=device)), dim=0)
    else:
        inv_freq = inv_freq_rotated
    inv_freq /= factor
    return (inv_freq, attention_factor)

def _compute_dynamic_ntk_parameters(config=None, device=None, seq_len=None, layer_type=None):
    """
    ...
    if seq_len is None:
        ...
        seq_len = torch.maximum(seq_len, torch.tensor(config.max_position_embeddings, dtype=seq_len.dtype, device=seq_len.device))
    else:
...
def _compute_yarn_parameters(config, device=None, seq_len=None, layer_type=None):
    """
...
def _compute_longrope_parameters(config, device=None, seq_len=None, layer_type=None):
    """
...
def _compute_llama3_parameters(config, device=None, seq_len=None, layer_type=None):
    """
    ...
    inv_freq = 1.0 / base ** (torch.arange(0, dim, 2, dtype=torch.int64).to(device=device, dtype=torch.float) / dim)
    factor = rope_parameters_dict['factor']
    low_freq_factor = rope_parameters_dict['low_freq_factor']
    high_freq_factor = rope_parameters_dict['high_freq_factor']
    old_context_len = rope_parameters_dict['original_max_position_embeddings']
...
ROPE_INIT_FUNCTIONS = {'linear': _compute_linear_scaling_rope_parameters, 'dynamic': _compute_dynamic_ntk_parameters, 'yarn': _compute_yarn_parameters, 'longrope': _compute_longrope_parameters, 'llama3': _compute_llama3_parameters, 'proportional': _compute_proportional_rope_parameters}
