# -*- coding: utf-8 -*-
# Generated by benchmark/suites/port.py from transformers.activations
# (transformers 5.13.0, source sha256 4b1a469c24be4e4c).  Do not edit; change the spec.
# Kept: logger, GELUTanh, PytorchGELUTanh, NewGELUActivation, GELUActivation, SiLUActivation, FastGELUActivation, QuickGELUActivation, ClippedGELUActivation, AccurateGELUActivation, MishActivation, LinearActivation, LaplaceActivation, ReLUSquaredActivation, SqrtSoftplusActivation, ClassInstantier, XIELUActivation, ACT2CLS, ACT2FN, get_activation, gelu
...
class GELUTanh(nn.Module):
    ...
    def __init__(self, use_gelu_tanh_python=False):
...
class ClippedGELUActivation(nn.Module):
    ...
    def __init__(self, min, max):
        if min > max:
            raise ValueError('min should be < max (got min: {0}, max: {1})'.format(min, max))
        super(ClippedGELUActivation, self).__init__()
        self.min = min
        self.max = max
...
class AccurateGELUActivation(nn.Module):
    ...
    def __init__(self):
        super(AccurateGELUActivation, self).__init__()
...
class ClassInstantier(OrderedDict):
    ...
    def __getitem__(self, key):
        content = super(ClassInstantier, self).__getitem__(key)
...
class XIELUActivation(nn.Module):
    ...
    def __init__(self, alpha_p_init=0.8, alpha_n_init=0.8, beta=0.5, eps=-1e-06, dtype=torch.bfloat16, with_vector_loads=False):
        super(XIELUActivation, self).__init__()
        ...
        try:
            ...
            try:
                ...
                msg += ' Could not enable torch._dynamo for xIELU ({0}) - this may result in slower performance.'.format(err)
                self._xielu_cuda_fn = self._xielu_cuda
            logger.warning_once(msg)
        except Exception as err:
            logger.warning_once('CUDA-fused xIELU not available ({0}) – falling back to a Python version.\nFor CUDA xIELU (experimental), `pip install git+https://github.com/nickjbrowning/XIELU`'.format(err))

    def _xielu_python(self, x):
        alpha_p = nn.functional.softplus(self.alpha_p)
        alpha_n = self.beta + nn.functional.softplus(self.alpha_n)
        return torch.where(x > 0, alpha_p * x * x + self.beta * x, (torch.expm1(torch.min(x, self.eps)) - x) * alpha_n + self.beta * x)

    def _xielu_cuda(self, x):
        ...
        result = self._xielu_cuda_obj.forward(x, self.alpha_p.to(x.dtype), self.alpha_n.to(x.dtype), self._beta_scalar, self._eps_scalar, self.with_vector_loads)
...
ACT2CLS = {'gelu': GELUActivation, 'gelu_10': (ClippedGELUActivation, {'min': -10, 'max': 10}), 'gelu_fast': FastGELUActivation, 'gelu_new': NewGELUActivation, 'gelu_python': (GELUActivation, {'use_gelu_python': True}), 'gelu_pytorch_tanh': GELUTanh, 'gelu_python_tanh': (GELUTanh, {'use_gelu_tanh_python': True}), 'gelu_accurate': AccurateGELUActivation, 'hardswish': nn.Hardswish, 'laplace': LaplaceActivation, 'leaky_relu': nn.LeakyReLU, 'linear': LinearActivation, 'mish': MishActivation, 'quick_gelu': QuickGELUActivation, 'relu': nn.ReLU, 'relu2': ReLUSquaredActivation, 'relu6': nn.ReLU6, 'sigmoid': nn.Sigmoid, 'silu': SiLUActivation, 'sqrtsoftplus': SqrtSoftplusActivation, 'swish': nn.SiLU, 'tanh': nn.Tanh, 'prelu': nn.PReLU, 'xielu': XIELUActivation}
...
def get_activation(activation_string):
    if activation_string in ACT2FN:
        ...
        raise KeyError('function {0} not found in ACT2FN mapping {1}'.format(activation_string, list(ACT2FN.keys())))
