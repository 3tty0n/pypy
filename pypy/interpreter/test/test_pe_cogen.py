...
def decode_all(co_code):
    ...
    while pc < len(co_code):
        opcode, bindings = pe_cogen.decode_instruction(code, pc)
...
def test_invalid_entry_pc_declines_instead_of_asserting():
    ...
    py.test.raises(BytecodeCorruption, pe_cogen.decode_instruction,
                   code, -1)
    py.test.raises(BytecodeCorruption, pe_cogen.decode_instruction,
...
def test_break_loop_keeps_enclosing_loop_across_handler_pop_block():
    ...
    pc = 0
    while pc < len(co_code):
        opcode, bindings = pe_cogen.decode_instruction(
...
def _break_loop_targets(func_code):
    ...
    pc = 0
    while pc < len(co_code):
        opcode, bindings = pe_cogen.decode_instruction(func_code, pc)
        if opcode == break_loop:
            targets.append(bindings["break_target"])
        pc = bindings["pc"]
    return targets
...
def test_nested_loops_resolve_to_their_own_loop():
    ...
    ends = pe_cogen._loop_ends(co_code)
    break_loop = bytecode_spec.opcodedesc.BREAK_LOOP.index
    seen = set()
    pc = 0
    while pc < len(co_code):
...
def test_gate_does_not_charge_the_estimate(monkeypatch):
    ...
    assert pe_cogen._gate_allows(profiler, code_size)
    assert pe_cogen._gate_allows(profiler, code_size)
    assert state.spent_ns == 0.0
    state.spent_ns = pe_cogen.GATE_BUDGET_FRACTION * 1e9
    assert not pe_cogen._gate_allows(profiler, code_size)
